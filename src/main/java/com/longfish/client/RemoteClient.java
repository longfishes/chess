...
import static com.longfish.CommonConstant.*;
...
public class RemoteClient extends JFrame {
    ...
    public RemoteClient(boolean isRed, String address, int port, int roomId, String myName, String myFace) {
        ...
        setTitle("中国象棋对战房间窗口");
        setSize(boardSizeX + 440, BoardSizeY + 213); // 调整窗口大小
        ...
        infoPanel.setPreferredSize(new Dimension(boardSizeX, 26)); // 设置固定高度
        ...
        blueSpacerPanel.setPreferredSize(new Dimension(boardSizeX, 60)); // 设置固定高度
        ...
        bottomPanel.setPreferredSize(new Dimension(boardSizeX, 66)); // 设置固定高度
        ...
        leftPanel.setPreferredSize(new Dimension(200, BoardSizeY));
        ...
        rightPanel.setPreferredSize(new Dimension(200, BoardSizeY));
    ...
    private class ChessPanel extends JPanel {
        ...
        private void drawPieces(Graphics g) {
            int pieceSize = (int) (cellSize * 0.9); // 缩小棋子大小
            int offsetX = (int) ((cellSize - pieceSize) * 2.4); // 水平中心对齐偏移
            ...
            for (int row = 0; row < boardHeight; row++) {
                for (int col = 0; col < boardWidth; col++) {
                    ...
                    if (piece != 0) {
                        ...
                        g.drawImage(piecesImages[player][pieceIndex], col * cellSize + offsetX, row * cellSize + offsetY, pieceSize, pieceSize, this);
                    }
                }
            }
        }
        ...
        private void drawSelection(Graphics g) {
            if (selectedPiece != null) {
                ...
                int offsetX = (int) ((cellSize - pieceSize) * 2.4);
                int offsetY = (cellSize - pieceSize) * 2;
                g.drawImage(selectImage, selectedPiece.x * cellSize + offsetX, selectedPiece.y * cellSize + offsetY, pieceSize, pieceSize, this);
            }
        }
            ...
            for (int row = 0; row < boardHeight; row++) {
                for (int col = 0; col < boardWidth; col++) {
                    if (validMoves[row][col]) {
                        ...
                        // 光点直径
                        int dotDiameter = cellSize / 4;
                        int offset = (cellSize - dotDiameter) / 2;
    ...
    private boolean connectToChatServer(int chatPort) {
        try {
            Socket chatSocket = new Socket(serverAddress, chatPort);
...
//            JOptionPane.showMessageDialog(this, "无法连接到聊天服务器！");
    ...
    private void exit() {
        // 重置房间
        int roomId = serverPort - MAIN_SERVER_PORT;
        try (Socket socket = new Socket(serverAddress, MAIN_SERVER_PORT)) {
        ...
        if (isOpen) {
            ...
            new Thread(() -> new MenuClient(serverAddress, myName, myFace, true)).start();
        }
    }
